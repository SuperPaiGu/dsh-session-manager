...
> **v0.2.7**：删除成功的会话立即从批量面板消失（本地过滤 + 列表刷新），无需重启。
...
**第 1 步**：到本仓库的 Release 页面下载 `dsh-session-manager-0.2.7.tgz`，然后在终端执行：

```sh
dsh plugin --profile web add ./dsh-session-manager-0.2.7.tgz
```
...
- **删除后立即从面板消失**：删除成功的会话会在批量面板里立刻移除，不用重启 DSH Web。

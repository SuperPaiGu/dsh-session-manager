...
中心列的最右边 **40px 被官方的「拖动改列宽」手柄占用**，而且它的抓取热区覆盖整条 40px 竖带、层级在视图之上。面板如果按普通布局铺到列边缘，行尾的「恢复 / 删除」按钮有 7px 落在这条带里 —— 鼠标一移上去就变成拖动光标，点不到按钮。

所以列表和底栏的右侧内边距是特意留出的（`padding-right:132px` / `150px`，行再加 50px），把按钮整体让到手柄左边。**这不是随手写的数字，是照着手柄热区量出来的**；如果以后官方改了手柄宽度或层级，这几个值要跟着调。左侧内边距则与官方对话视图一致（32px），保证视觉中线对齐。

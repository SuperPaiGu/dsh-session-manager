...
中心列的最右边 **40px 被官方的「拖动改列宽」手柄占用**，而且它的抓取热区覆盖整条 40px 竖带、层级在视图之上。面板如果按普通布局铺到列边缘，行尾的「恢复 / 删除」按钮会落进这条带里 —— 鼠标一移上去就变成拖动光标，按钮点不动。

**关键坑：手柄距面板右边缘的距离会随列宽变化**，所以固定的右边距必然会在某个窗口尺寸下失效：
...
所以列表和底栏用的是**比例内边距** `calc(120px + 12%)`，跟着列宽一起缩放；窄列时自动收缩，不会把标题挤没。左侧保持与官方对话视图一致的 32px 缩进，保证视觉中线对齐。

**这几个数字不是随手写的，是逐像素 `elementFromPoint` 量出来的**。如果以后官方改了手柄的宽度、位置或层级，要重新量：正确判据是——**面板里每个按钮的每一个像素，最顶层元素都必须是它自己**（可用 `elementFromPoint` 沿按钮那一行逐像素扫描验证）。

...
主服务和辅助端共用 `~/.dsh` 时，**归档名单有两份内存副本**（每个进程各一份），它们的写入会互相覆盖：

- 在 A 实例归档/恢复后，B 实例的界面不会自动更新，B 的下一次写入还会把 A 的改动覆盖掉
- 插件的恢复是直接改磁盘那份（唯一可行的路径），所以**跨实例操作后要重启另一个实例**才能看到一致结果
- 面板里的「归档区 N 个」会**每 4 秒自动重读一次**，所以磁盘变了它最多 4 秒后自己纠正

**建议：归档管理只在其中一个实例里做**（比如就固定在桌面端那个），不要在两边来回切。
